...
The fact that definiteness is not morphologically indicated in Russian as it is in English means that diagnosing anti-uniqueness effects is more difficult. DP-internal \textit{only} is generally barred from indefinite NPs in English, as (\ref{indef-only})-(\ref{indef-only2}) show, although it can be used in the phrase \textit{an only child}, which seems to be an idiom.

\begin{exe}
	\ex[??] { \label{indef-only} Scott is not an only author of Waverley. }
	\ex[??] { \label{indef-only2} Anna didn't score an only goal. }
\end{exe}

However, DP-internal \textit{only} in Russian, which is pronounced as \textit{edinstvennyj}, may be interpreted as indeterminate:

\begin{exe}
	\ex \label{sole-director2} \gll U \`{e}toj kompanii --- edinstvennyj direktor.\\
	At this company {} only director\\
	\glt `This company has a sole director.'

	\ex \label{single-good-essay2} \gll Marija napisala edinstvennoe xoro\v{s}oe so\v{c}inenie za vsju \v{z}izn'.\\
	Maria wrote only good essay in entire life\\
	\glt `Maria wrote a single good essay in her entire life.'

	\ex \label{single-good-speech2} \gll Boris ne proizn\"{e}s ni edinstvennoj xoro\v{s}oj re\v{c}i na svad'be.\\
	Boris not gave not only good speech at wedding\\
	\glt `Boris didn't give a single good speech at the wedding.'

	\ex \label{only-child-ru2} Marija --- edinstvennyj reb\"{e}nok.\\
	Maria {} only child
...
% TODO: Parenthetical citation of C+B (2012a) is a little awkward here.

In this respect, \textit{edinstvennyj} behaves more similarly to the English words \textit{sole} and \textit{single}, which, as the glosses indicate, can appear in indefinite NPs in English \citep{cb2012a}.

% TODO: Note how you can't tell morphologically if an NP is definite or not in Russian, and introduce evidence that edinstvennyj can be indeterminate.
